...
RayIntersection VoxelShape::raycast(glm::vec3 const &origin, glm::vec3 const &direction, float const &length)
{
    ...
    glm::vec3 dir = glm::vec3(collider->getInverseGlobalMatrix() * glm::vec4(direction, 0.0f));

    glm::vec3 localOrigin = glm::vec3(collider->getInverseGlobalMatrix() * glm::vec4(origin, 1.0f))- glm::vec3(voxelSize * 0.5);
